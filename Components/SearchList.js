...
const SearchList = ({content, images, navigation, filter, prev, route, changePrev}) => {
    ...
    const [isSearchActive, changeIsSearchActive] = useState(false)
        ...
        let searchData = []
        let prevData = []
        let searchDataLength = route != undefined ? `${route.params.searchDataLength}` : 0
        ...
        for(let i = 0; i < content.length; i++){
			if(content[i].title.search(searchFilter) !== -1){
                ...
                searchDataLength = searchDataLength == 0 ? searchDataLength++ :  searchDataLength
                ...
                <Toolbar
                    ...
                    searchable={{
                        ...
                        onSubmitEditing: (() => {
                            ...
                            changePrev({             
                                title: searchFilter == `${/(?:)/i}` ? 'Все рецепты' : `${searchFilter}`.replace(/\//,'').replace(/\/i/,''),
                                data: newSearchData
                            })

                            return navigation.navigate('Поиск', {
                                searchData: newSearchData,
                                title: `"${searchFilter}"`.replace(/\//,'').replace(/\/i/,''),
                ...
                renderItem = {({item}) => {
                    if(isSearchActive){
                        return(
                            ...
                            >
                                <ListItem 
                                    leftElement = {/\d{1,3}p/.test(`${item.id}`) ? 'refresh' : 'search'}
